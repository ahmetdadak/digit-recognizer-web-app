...
def crop_image(img_ndarray):
    ...
    # Across cols
    first_column = np.nonzero(img_ndarray)[1].min()
    last_column = np.nonzero(img_ndarray)[1].max()

    # Crop by longest non-zero to make sure all is kept
    
    cropped = img_ndarray[first_row:last_row+1, first_column:last_column+1]
    # Crop image
    return cropped

def center_image(img_ndarray):
    """
